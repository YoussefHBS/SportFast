...
import { Cliente } from '../modelos/cliente'; // Asegúrate de importar tu modelo
...
export class AuthService {
  ...
  login(data: any) {
    return this.http.post<{ token: string; cliente: Cliente }>('http://localhost:8000/api/login', data).pipe(
      tap((res) => {
  ...
  getUser() {
    ...
    return this.http.get<Cliente>('http://localhost:8000/api/user', { headers }).pipe(
      tap((user) => {
  ...
  logout() {
    ...
    return this.http.post('http://localhost:8000/api/logout', {}, { headers }).pipe(
      tap(() => {
        this.token = null;
